...
- [x] **T10** ✅ — `POST /api/v1/products/images` endpoint trong `api/v1/products.py`
- [x] **T11** ✅ — `DELETE /api/v1/products/images/{image_id}` endpoint
- [x] **T12** ✅ — Mount StaticFiles `/uploads` trong `main.py`

## Backend — Tests

- [x] **T13** ✅ — pytest fixtures: `make_test_image(size)` helper trong `tests/conftest.py`
- [x] **T14** ✅ — `tests/test_product_images.py`: happy + 401 + oversized + invalid MIME
- [x] **T15** ✅ — `tests/test_product_images.py`: malformed image + delete + EXIF rotation
...
- [x] **T27** ✅ — Update `ProductCard.tsx` + `ShopPage.tsx` dùng `urls.medium` cho thumbnail
